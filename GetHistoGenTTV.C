...
// Delare histonames and bins.
  ...
  histonames.push_back("gentop_pt");  histobinn.push_back(85); histobinl.push_back(0.); histobinu.push_back(1000.);
...
/*      for (int i=0;i<cms3.gengs_p4().size();i++) {
        if(fabs(cms3.gengs_motherid().at(i))==6){
         gs_fromT.push_back(gengs_p4().at(i));
       }
      }
  */  //find b and bbar
...
//   find the neutrinos from Z
    ...
    if(is1l){
    if(zboson.pt() > 100 && nus_fromZ.size()==2) value["genz_pt"] = zboson.pt();
