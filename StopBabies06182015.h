// -*- C++ -*-
#ifndef StopBabies06182015_H
#define StopBabies06182015_H
...
protected: 
	...
	bool M3b_isLoaded;
	float	M3b_lep2_;
	TBranch *M3b_lep2_branch;
	bool M3b_lep2_isLoaded;
	int	HLT_SingleEl_;
...
void Init(TTree *tree) {
  ...
  tree->SetMakeClass(1);
	...
	M3b_branch = 0;
	if (tree->GetBranch("M3b") != 0) {
		M3b_branch = tree->GetBranch("M3b");
		if (M3b_branch) {M3b_branch->SetAddress(&M3b_);}
	}
	M3b_lep2_branch = 0;
	if (tree->GetBranch("M3b_lep2") != 0) {
		M3b_lep2_branch = tree->GetBranch("M3b_lep2");
		if (M3b_lep2_branch) {M3b_lep2_branch->SetAddress(&M3b_lep2_);}
	}
	...
	// load all branches
{
	...
	float &M3b()
	{
		if (not M3b_isLoaded) {
			if (M3b_branch != 0) {
				M3b_branch->GetEntry(index);
			} else { 
				printf("branch M3b_branch does not exist!\n");
				exit(1);
			}
			M3b_isLoaded = true;
		}
	...
	float &M3b_lep2()
	{
		if (not M3b_lep2_isLoaded) {
			if (M3b_lep2_branch != 0) {
				M3b_lep2_branch->GetEntry(index);
			} else { 
				printf("branch M3b_lep2_branch does not exist!\n");
				exit(1);
			}
			M3b_lep2_isLoaded = true;
		}
		return M3b_lep2_;
	}
	int &HLT_SingleEl()
...
#ifndef __CINT__
extern StopBabies06182015 cms3;
#endif
