...
void makePlotOneCut(){
 ...
 // string poststring[poststringsize] = {"SR","SR200","SR200MT2W"};
  ...
  for(unsigned int n=0; n<bgsetsize; ++n) {
    ...
    TString x = ("rootfiles/CutHistos/StackPlots"+datasetstring+".root");
    cout << x << endl;
      fbg[n] = TFile::Open(x);
      //  cout << fbg[n].IsZombie();
  }
  ...
  string outputdir = "rootfiles/CutPlots/StackPlots/";
  ...
  TH1F* null = new TH1F("","",1,0,1);
    ...
    for(unsigned int i = 0; i<histonames.size();++i){
      ...
      string options = "--outputName " + outputdir + prestring + "/" + histonames[i] + " --xAxisLabel " + histonames[i] + " --energy 13 --lumi 10 --legendTextSize 0.02 --type 1l --preserveBackgroundOrder --legendUp 0.05";
